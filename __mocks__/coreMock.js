export const test_data = {
  from: 'mnemonic',
  mnemonic: 'Spawn fade knife prison oak absurd also proud refuse bitter job nephew'
  // mnemonic: 'magic disorder cable gym napkin gather relief vapor daring main capable armor'
}

export const test_derivation_path = `m/44'/0'/0'/0`

export const test_core = {
  'BCH': {
    'externalAddress': '1L7eVG7xQtrMqZajfs7Lzyuu9MxJixqK8u'
  },
  'BTC': {
    'p2pkh': {
      'externalAddress': '1NrHVrLqZy7jGByMhhtUjht9De4hgZ6xsj'
    },
    'p2wpkh': {
      'externalAddress': 'bc1qa40ruah2fkr7xh9n8g948ryx9jcltkj83glyr8'
    }
  },
  'ETH': {
    'externalAddress': '0x9f29574bf4c1e5a2f5c38fdf93d3cf9ca00a66aa'
  },
  'BTCV': {
    'externalAddress': 'royale1qwvd3knuqk9n6xl4773dvag4qav7amurwd43pu8'
  },
  'DOGE': {
    'p2pkh': {
      'externalAddress': 'DLXryK9F7kUv8Y1KmMMSnuj1NBMrcqFMci'
    }
  },
  'count': undefined,
  'from': 'mnemonic',
  'mnemonic': 'magic disorder cable gym napkin gather relief vapor daring main capable armor',
  'xprv': 'xprv9s21ZrQH143K3weiVrc21GMaWi3cGckimJHubnAK5QPicKHpv6DF92WLiWpPJ7cMwLMMNrzthhHymesLPx8xGbbMvTQgyHPUufzE7RLmqYb'
}

export const test_child_nodes = {
  node:
    {
      publicExtendedKey:
        'xpub6Do8cbHpTg5AepKzV4Tjxepa8zZchVBcN3oba4AZkxsB77hhEFRPybUbJYEdUD4JJQJx6yf6AniAtnZfV3kqAXn3Mbarzb7msdTNPUcFBfu'
    },
  list:
    [
      {
        path: 'm/44\'/0\'/0\'/0/0',
        publicKey:
          '0321e8aa81fa978173588bb1b7cd0d68a4348472da27bd6846caf0098095629326',
        p2pkhAddress: '1NrHVrLqZy7jGByMhhtUjht9De4hgZ6xsj',
        ethAddress: '0xb27bc049c4e1c443153259958149e715f6aab88e',
        bchAddress: 'qrh6ejupsvfn7rzpyg9edsd8sxx2t5w0fv69a7yw0z',
        btcvAddress: 'royale1qa7kvhqvrzvlscsfzpwtvrfup3jjarn6th8dx20',
        dogeAddress: 'DSzP37HUsP21oC9xSHt3HU3k6mnzyXvMu4'
      },
      {
        path: 'm/44\'/0\'/0\'/0/1',
        publicKey:
          '036ea855e01dbe50e6db58b1708cfac3984479ca38a2f12c57cc20816c03b06fed',
        p2pkhAddress: '1Q2XXJPC6J36rjDTptbxEJyn8h31V6vjee',
        ethAddress: '0x2759e8446bf4dab547a8e345523547021431d4da',
        bchAddress: 'qr7ffjdwzrkunnzz0weums60sq4h8ph97sp4hek49n',
        btcvAddress: 'royale1qlj2vntssahyucsnmk0xuxnuq9decde05scd8w3',
        dogeAddress: 'DUAd4ZKqPhwPPjQ4ZUbWn59P1pmJnsJ8oV'
      },
      {
        path: 'm/44\'/0\'/0\'/0/2',
        publicKey:
          '03987100e398f108207aab95445694fbed92738b0aaa5e9c5c6d11601ba31d364b',
        p2pkhAddress: '1Mw2mEUeL4Ao4cBB3HdpT91mVMbxRK6PiG',
        ethAddress: '0x5d03fde7c3c75a2345fe597fe526ff38a978a85f',
        bchAddress: 'qrje5uhsfzewuhj8kftdy436frnhj9x76u29k4uzuk',
        btcvAddress: 'royale1qukd89uzgkth9u3aj2mf9vwjguau3fhkhn5lwh2',
        dogeAddress: 'DS58JVRHdU55bcMmmsdNzuBNNVLFo4UaTn'
      },
      {
        path: 'm/44\'/0\'/0\'/0/3',
        publicKey:
          '02891718d22ae2408885ea09d51096b4f4646a13ce576c3ab51142cbf7e830867f',
        p2pkhAddress: '1Ge8oR98J9mcWdZHfn5CL1FZgHadpwZhXZ',
        ethAddress: '0x5bc54d20abce2d4041082503b9774ba5f19d2f7a',
        bchAddress: 'qz4clj080y32zgffptf0nr8f86lslt6zgcyqh38l6h',
        btcvAddress: 'royale1q4w8unemey2sjz2g26tuce6f7hu867sjxws7tla',
        dogeAddress: 'DLnELg5mbZfu3djtQN4ksmRAZRJwDGGLTf'
      },
      {
        path: 'm/44\'/0\'/0\'/0/4',
        publicKey:
          '03419323f21346edcefc1fd1fdaba31069f51fbeca4558a928a151df71b7a5bd78',
        p2pkhAddress: '1NN9JCmZfYfWjg2JSeSPNvSYL5kxZ1kBZ6',
        ethAddress: '0x28eb0dbf350e1120930925a01f3f53cdd23108c6',
        bchAddress: 'qr495ntygkc5r6702teu2jg375p8kqekkgjcp6fc06',
        btcvAddress: 'royale1qafdy6ez9k9q7hn6j70z5jy04qfasxd4jlw4sxv',
        dogeAddress: 'DSWEqTiCxxZoGgCuBERwvgc9DDVFqFZnqQ'
      }
    ]
}


